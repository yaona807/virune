...
- all three JavaScript interop tiers: generated safe binding, compiled TypeScript adapter, and isolated audited unsafe FFI.
...
`src/ffi/unsafe-hostname.virune` is the **unsafe FFI** example. The raw `unsafe extern` remains inside an `unsafe module` under `src/ffi/`; normal Virune code can only see the audited native-shaped facade rather than the raw JavaScript symbol. This example deliberately demonstrates the trust boundary rather than weakening it.

## Public API snapshot

`node/virune.api.json` is generated by `virune api` and checked in. It records only public Virune declarations in deterministic module/declaration order. `--check` fails when the source API and the checked-in snapshot drift.
...
Continuous Node/browser/binding/API drift enforcement belongs to Issue #81; this directory is the canonical source material that gate will execute.
