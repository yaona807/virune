import { Err, Ok, type JsError, type Result, toJsError, type TaskContext } from '@virune/runtime';
...
export async function request(
	...
	try {
		...
		return Ok({ status: response.status, ok: response.ok, headers: new Map(response.headers.entries()), body: textBody(await response.text()) });
