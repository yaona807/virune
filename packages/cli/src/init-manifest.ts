...
export function generatedProjectDependencyVersions(version: string): {
	...
	if (compareCoreVersion(parsed, FIRST_REGISTRY_RELEASE) >= 0) {
...
function parseReleaseVersion(version: string): { readonly major: number; readonly minor: number; readonly patch: number } {
