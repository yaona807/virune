...
function printViewChild(child: ViewChild, indent: number): string {
	...
	switch (child.kind) {
		...
		case 'ViewRepetition': return `${prefix}for ${child.itemName}${child.indexName === undefined ? '' : `, ${child.indexName}`} in ${printExpression(child.source)}${child.identity === undefined ? '' : ` by ${printExpression(child.identity)}`} ${printViewBlock(child.body, indent)}`;
	}
}
