...
export class ProjectManager {
	...
	readonly #projectRevisions = new Map<string, number>();
	#revision = 0;
	...
	public invalidateProject(root: string, options: ProjectInvalidationOptions = {}): void {
		...
		this.#projectRevisions.set(normalizedRoot, (this.#projectRevisions.get(normalizedRoot) ?? 0) + 1);
	...
	async #analyzeCore(uri: string, scope: AnalysisScope, token?: AnalysisCancellationToken): Promise<ResolvedAnalysis | undefined> {
		...
		const projectRevision = this.#projectRevisions.get(root) ?? 0;
		...
		if (this.#revision === revision && (this.#projectRevisions.get(root) ?? 0) === projectRevision) {
