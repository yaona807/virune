...
test('valid empty project flows through parse, link, semantics, lowering, and deterministic emission', async () => {
	await withGeneratedCompiler((module, input) => {
		...
		assert.ok(first.emittedModules.every(item => item.code.startsWith(`${BASE_RUNTIME_IMPORT_LINE}\n`)));
		assert.ok(first.emittedModules.every(item => !/Generated by Virune self-host/u.test(item.code)));
