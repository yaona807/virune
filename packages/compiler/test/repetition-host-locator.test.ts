...
test('repetition Host locator accepts the prototype extern declaration and preserves declaration provenance', () => {
	const result = compileSource(source('@repetitionHost("render", 1)\nextern js "./host.js" {}\n'), { emit: false });
	assert.deepEqual(errorCodes(result), []);
	assert.ok(result.ast);
	const resolution = discoverRepetitionHostLocator([{ path: '/project/src/host-locator.virune', ast: result.ast }]);
	...
	assert.equal(resolution.locator.declarationFile, '/project/src/host-locator.virune');
});
...
test('repetition Host locator discovery returns none without a valid locator', () => {
	...
	assert.deepEqual(errorCodes(result), []);
	assert.ok(result.ast);
	assert.deepEqual(discoverRepetitionHostLocator([{ path: '/project/src/value.virune', ast: result.ast }]), { status: 'none' });
});
