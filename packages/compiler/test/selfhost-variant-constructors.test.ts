...
test('Option and Result constructors resolve from expected types and execute', async () => {
	...
	try {
		...
		assert.match(emittedCode, /\$tag: "Some"/);
		assert.match(emittedCode, /\$tag: "None"/);
		assert.match(emittedCode, /\$tag: "Ok"/);
		assert.match(emittedCode, /\$tag: "Err"/);
		assert.match(emittedCode, /\$values: \[7\]/);
		assert.match(emittedCode, /\$values: \[13\]/);
		assert.match(emittedCode, /\$values: \[17\]/);
		assert.match(emittedCode, /\$values: \[19\]/);
		assert.doesNotMatch(emittedCode, /\b(?:Some|None|Ok|Err)\s*\(/);
