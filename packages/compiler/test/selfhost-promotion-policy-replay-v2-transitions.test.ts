...
function entry(
	...
	return {
		...
		evidence: requiredEvidence.map((id, index) => ({
			...
			sha256: `${index.toString(16).padStart(2, '0')}${'e'.repeat(62)}`,
		})),
	};
}

test('counting subject transition does not inherit the previous subject streak', () => {
	const result = replayPromotionHistoryAgainstPolicyV2(
		...
		'required-selfhost',
		{
			...
			entries: [
				...
				entry('run-2', '2026-08-19T01:00:00.000Z', subjectA),
				entry('run-3', '2026-08-20T01:00:00.000Z', subjectB),
			],
		},
	);
	assert.equal(result.promotionSubjectId, subjectB);
...
test('a product failure remains invalidating when the same subject returns later', () => {
	const result = replayPromotionHistoryAgainstPolicyV2(
		policy(),
		'required-selfhost',
		{
			...
			entries: [
				...
				entry('run-2', '2026-08-19T01:00:00.000Z', subjectB),
				entry('run-3', '2026-08-20T01:00:00.000Z', subjectA),
			],
		},
	);
	assert.equal(result.promotionSubjectId, subjectA);
	assert.equal(result.productInvalidated, true);
	assert.equal(result.unexplainedDifferentials, 1);
	assert.deepEqual(result.qualifyingRunIds, []);
