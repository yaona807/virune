...
const trailingParameterCommaSource = `fn add(
	left: Int,
	right: Int,
) -> Int {
	return left + right
...
test('multiline function parameters accept one trailing comma', async () => {
	const loaded = await loadMvpModule();
	try {
		const output = await createSelfhostMvpKernel(loaded.module).compile(input(trailingParameterCommaSource));
		...
		assert.match(emittedCode, /function add/u);
...
test('an empty parameter between commas remains rejected', async () => {
	const loaded = await loadMvpModule();
	try {
...
async function loadMvpModule(): Promise<{ readonly root: string; readonly module: SelfhostMvpModule }> {
	...
	const root = await mkdtemp(join(temporaryRoot, 'selfhost-trailing-parameter-comma-'));
