...
export class AstBuilder extends baseCstVisitorConstructor {
	...
	public contextualAggregateExpression(ctx: Ctx): A.ContextualAggregateExpression { return { id: this.id(), kind: 'ContextualAggregateExpression', span: nodeSpan(this.#fileId, this.currentNode(ctx)), entries: this.visitNodes(nodes(ctx, 'contextualAggregateEntry')) }; }
	public contextualAggregateEntry(ctx: Ctx): A.ContextualAggregateEntryNode { return { name: tokenText(ctx, 'IdentifierName'), value: this.visitNode(firstNode(ctx, 'expression')), span: nodeSpan(this.#fileId, this.currentNode(ctx)) }; }
