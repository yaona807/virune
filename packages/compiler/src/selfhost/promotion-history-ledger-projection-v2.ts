...
export function projectPromotionHistoryLedgerV2(value: unknown): PromotionHistoryLedgerProjectionV2 {
	...
	if (effectiveRuns.length === 0) {
		throw new PromotionHistoryLedgerProjectionError('promotion history ledger has no formal runs to project');
	}
	const entries = effectiveRuns.map((run, index) => projectEffectiveRun(
		run,
		ledger.stage,
		ledger.runs[index]!.executionCommit,
	));
...
function projectEffectiveRun(
	...
	const gapDigest = sha256(serializedGap);
	return {
		version: 2,
		runId: run.runId,
		stage,
		executionCommit,
		promotionSubjectId: gapSubjectId(serializedGap),
		completedAt: run.sequenceAt,
		outcome: run.reason === 'workflow-cancelled' ? 'cancelled' : 'infrastructure-failed',
...
function gapSubjectId(serializedGap: string): string {
	return sha256(`promotion-history-ledger-gap-subject-v2\n${serializedGap}`);
}
