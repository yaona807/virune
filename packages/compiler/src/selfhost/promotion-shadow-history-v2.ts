...
export function createPromotionShadowHistoryV2(value: unknown): PromotionShadowHistoryResultV2 {
	...
	const countingEntries = entries.filter(entry => entry.countsTowardPromotion);
	const currentSubjectId = countingEntries.at(-1)?.promotionSubjectId ?? latest.promotionSubjectId;
	const currentSegment = countingEntries.length === 0
		? []
		: trailingSubjectSegment(countingEntries, currentSubjectId);
	const countingCurrentSubjectEntries = countingEntries.filter(entry => entry.promotionSubjectId === currentSubjectId);
	const productInvalidated = countingCurrentSubjectEntries.some(entry => entry.outcome === 'product-failed');
	const unexplainedDifferentials = countingCurrentSubjectEntries.reduce((total, entry) => {
		const next = total + entry.unexplainedDifferentials;
		if (!Number.isSafeInteger(next)) {
			throw new PromotionShadowHistoryError('$.entries', 'aggregate unexplained differentials exceed safe integer range');
		}
		return next;
	}, 0);
...
function parseEntry(
	...
	if (outcome !== 'product-failed' && unexplainedDifferentials !== 0) {
		throw new PromotionShadowHistoryError(
			`${path}.unexplainedDifferentials`,
			'only product-failed observations may have unexplained differentials',
		);
	}
