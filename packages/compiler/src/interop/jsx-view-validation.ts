...
function renderExternalViewPropertyValue(expression: A.Expression, context: RenderContext): string | undefined {
	...
	return renderCheckedExternalPropertyExpression(
		expression,
		context,
...
function renderCheckedExternalPropertyExpression(expression: A.Expression, context: RenderContext, failureMessage: string, depth = 0): string | undefined {
	...
	if (expression.kind === 'IdentifierExpression' && expression.symbolId !== undefined) {
		...
		if (symbol?.kind === 'variable' && !symbol.mutable) {
			...
			if (declaration?.kind === 'LetStatement') {
				const letStatement = declaration as A.LetStatement;
				if (letStatement.annotation === undefined) return renderCheckedExternalPropertyExpression(letStatement.value, context, failureMessage, depth + 1);
			}
		}
	}
	if (expression.kind === 'FieldExpression') {
		const target = renderCheckedExternalPropertyExpression(expression.target, context, failureMessage, depth + 1);
		return target === undefined ? undefined : `${target}.${expression.field}`;
	}
	...
	if (expression.kind === 'CallExpression' && (expression.foreignCall === true || expression.foreignConstruct === true) && expression.typeArguments.length === 0) {
		...
		for (const argument of expression.arguments) {
			const rendered = renderCheckedExternalPropertyOperationArgument(argument, context, failureMessage, depth + 1);
			if (rendered === undefined) return undefined;
...
function renderCheckedExternalPropertyOperationArgument(expression: A.Expression, context: RenderContext, failureMessage: string, depth: number): string | undefined {
	const typeId = expression.inferredTypeId;
	...
	if (type.kind === 'foreign') {
		...
		return renderCheckedExternalPropertyExpression(expression, context, failureMessage, depth);
	}
