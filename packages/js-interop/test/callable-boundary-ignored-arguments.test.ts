...
test('provided optional callback parameter may ignore variadic any event arguments', async () => {
	const result = await compileCase(
		'export declare function off(event: "active", fn?: (...args: any[]) => void): void;\n',
		`import js { off } from "./library.js"\n\nfn handler() -> Unit {\n\treturn Unit\n}\n\nfn main() -> Unit uses JavaScript {\n\tdiscard off("active", handler)\n\treturn Unit\n}\n`,
	);
	assert.deepEqual(errors(result), []);
	const projection = result.semantic?.interop.callableProjections?.[0];
	assert.ok(projection);
