...
test('Repetition Host whole-usage validation rejects missing and incompatible exports', async () => {
	const cases: Array<[string, string | undefined]> = [
		...
		['Host result incompatible with body View', 'export declare function render<T>(readSnapshot: () => ReadonlyArray<{ id: string; index: number; value: T }>, renderGroup: (readValue: () => T, readIndex: () => number, id: string) => JSX.Element): number;\n'],
