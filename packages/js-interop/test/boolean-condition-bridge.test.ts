...
test('non-boolean and ambiguous JavaScript evidence cannot bridge to Bool', async () => {
	...
	const source = sourceFor(
		'import js { state } from "./library.js"',
		'',
		'fn rejectAny() -> Unit uses JavaScript {',
		'\tif state.raw {',
		'\t\tdiscard 0',
		'\t}',
		'}',
		'fn rejectUnknown() -> Unit uses JavaScript {',
		'\tif state.uncertain {',
		'\t\tdiscard 0',
		'\t}',
		'}',
		'fn rejectNumber() -> Unit uses JavaScript {',
		'\tif state.count {',
		'\t\tdiscard 0',
		'\t}',
		'}',
		'fn rejectString() -> Unit uses JavaScript {',
		'\tif state.text {',
		'\t\tdiscard 0',
		'\t}',
		'}',
		'fn rejectUnion() -> Unit uses JavaScript {',
		'\tif state.mixed {',
		'\t\tdiscard 0',
		'\t}',
		'}',
...
test('stale property snapshots cannot bridge and missing evidence fails closed', async () => {
	...
	const unresolved = compileCondition(root, sourceFor(
		...
		'fn unresolvedCondition() -> Unit uses JavaScript {',
		'\tif missing {',
		'\t\tdiscard 0',
		'\t}',
		'}',
