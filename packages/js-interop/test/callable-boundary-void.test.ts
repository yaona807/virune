...
test('TypeScript Promise<void> accepts an async native Unit callback without changing async semantics', async () => {
	...
	assert.match(result.output?.code ?? '', /async \(\) => \{ try \{ return encodeFfiValue\(await \$fn\(rootTaskContext\(\)\), \{ kind: 'undefined' \}\); \} catch \(\$error\) \{ throw \$viruneExternalizeInteropError\(\$error\); \} \}/u);
});
