...
The Runtime and Standard Library package surfaces bind the package name/version, module type, Node engine contract, executable exports, runtime dependencies, and the complete built `dist/src` artifact tree. Every executable export must resolve to a regular non-symlink file inside that bound `dist/src` tree; an export outside it, a missing target, or a symlink target fails closed instead of leaving executable bytes outside product identity. Package metadata that can affect runtime/module resolution but is not yet modeled explicitly—`main`, `module`, `browser`, `imports`, `sideEffects`, optional or peer dependency metadata, and `os`/`cpu`/`libc` constraints—fails closed if introduced. Documentation/repository metadata, keywords, build scripts, and development-only dependencies remain outside product identity; any build effect they cause is still reflected by the bound built artifact bytes.

The Host component keeps the execution/selection boundary explicit rather than hashing the entire compiler or `selfhost` directory. Every direct runtime import from the fixed Host root set must resolve to another fixed Host root, the separately bound bootstrap-policy component, the Stage-0 project-build closure, or a Node built-in. The existing Legacy adapter is itself a fixed Host root because current compiler code imports it statically; promotion tooling does not alter compiler loading semantics to exclude it. A newly introduced import outside those bindings fails closed instead of silently becoming an untracked dependency.
...
Because the current compiler statically imports the Legacy adapter from Host boundary modules, the conservative Promotion Subject binds that adapter as part of the fixed Host set. This can reset the Self-host observation subject when Legacy adapter bytes change, but it avoids changing compiler execution structure merely to optimize promotion identity. The product boundary can be narrowed only through an independently justified compiler architecture change, not through promotion evidence tooling.
...
Performance evidence compares Legacy and Self-host Project Compiler execution on the complete non-divergent project corpus using the existing Self-host Gate D numeric ratios. The current edited-rebuild measurement is explicitly a proxy and does not prove incremental-cache performance. Because Gate D requires a real incremental-build comparison, this proxy cannot produce passing `performance-smoke` or `performance-budget` evidence: the canonical observation remains `product-failed` until comparable incremental evidence is available. Promotion tooling does not change the compiler or weaken Gate D merely to make this evidence pass.

Each evidence record carries canonical command/environment metadata and SHA-256 digests. The final assembler revalidates release-core and cross-runner self-hashes, Promotion Subject canonicalization, quality evidence self-hashes, the performance corpus and Gate D claim, the current policy evidence set, and measured performance ratios instead of trusting top-level status strings. Before writing the artifact, it also replays the canonical observation against the current blocking promotion-policy safety contract. A weakened threshold, unsupported blocking-policy field, or other invalid current policy therefore fails closed without producing a canonical observation.
