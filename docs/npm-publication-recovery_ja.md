...
| bytes、version、provenance、repository/source identity、canonical tag、その他reviewed identityのいずれかが不一致 | immutable package version自体を変えない独立した外部設定エラーとして修復可能な場合を除き、**そのpackage versionの再利用を永久に禁止**する。 |
...
retryで既存の`name@version`をskipできるのは、bytes、provenance/source identity、canonical tagがすべてreview済みreleaseと一致した場合だけです。canonical tagが不一致または外部でdriftしている場合、normal pathは停止します。recoveryでは **別の`npm dist-tag` mutationを使わない** うえ、tag修復のための **traditional token fallback** も導入しません。この状態はTrusted Publishing境界を暗黙に弱めず、明示的な外部調査対象とします。
