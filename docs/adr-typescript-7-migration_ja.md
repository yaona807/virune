...
2. `@virune/js-interop`がruntimeでJavaScript／TypeScript declarationを解決するために組み込む同期Compiler API
...
PR #26ではroot dependencyをTypeScript 6.0.3から7.0.2へ直接更新し、build系の全Workflowが失敗しました。この更新は`typescript`としてimportするpackageを置換したため、interop providerが必要とするJavaScript APIの喪失とbuild toolchain変更が同時に発生しました。
...
ProductionのCompiler API importは`packages/js-interop/`に限定します。中心となる実装は`packages/js-interop/src/index.ts`で、providerは`Program`、`TypeChecker`、`LanguageService`、module resolution、diagnostic、TypeScript AST predicateを使用します。同一packageのtestは制御されたfixtureを構築する目的でTypeScriptをimportできます。

Compiler、runtime、CLI、formatter、standard library、language server、VS Code packageから`typescript` JavaScript APIを直接importしてはいけません。Boundary verifierは許可package外のimportまたはdependency追加を拒否します。

## Decision

Build compilerと組み込みCompiler APIを別packageとして管理します。

移行PRでは次の厳密なdependency構成を使用します。
...
`@typescript/native`はrepository build scriptが使用するnative `tsc` executableを提供します。`typescript` import specifierは`@virune/js-interop`用のTypeScript 6 compatibility packageへ解決し続けます。Interop workspaceも同じexact aliasを保持し、公開dependency graphがnpm hoistingへ依存しないようにします。

Version rangeは使用せずexact versionにします。

- native compilerは全CIとstable release証跡を伴うreview済みdependency PRで更新する
- compatibility APIは独立して更新し、binding corpus reviewを必須とする
- 2つのversionは同じrelease cadenceで更新する必要がない
- TypeScript 7.1以降のprogrammatic APIをVirune provider contractに対して評価するまで、compatibility APIを置換しない
...
1. TypeScript 6 Compiler API境界とversion
...
3. 2-package dependency構成とlockfile更新を適用する実装PRを作成する。
...
Rollbackは単一dependency変更で行います。`@typescript/native`を削除し、`typescript`をexact `6.0.3`へ戻し、lockfileを再生成してstable release gateを実行します。Compiler API import境界はrollback中も変更しません。

## Consequences

- 同期interop providerを書き直さずnative compilerを利用できる。
...
- Boundary policyによりJavaScript interop外からlegacy APIへ偶発的に依存することを防止できる。
