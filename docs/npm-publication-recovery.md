...
Workflow-owned release publication runs are serialized across release refs with the native GitHub Actions concurrency primitive. A missing candidate is not published from the initial complete-set observation alone: immediately before each write, the candidate and its exact Virune package dependencies are reobserved. If the candidate became exact, provenance is verified and the write is skipped; if a dependency, candidate, or canonical tag state became unknown or contradictory, the write is blocked.
