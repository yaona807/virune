...
Host componentはCompiler全体や`selfhost` directory全体をhashせず、実行・選択境界を明示した固定root集合として保持します。固定Host rootからの直接runtime importは、別の固定Host root、別componentとしてbind済みのbootstrap policy、Stage 0 project build closure、Node built-in、またはversion付きで明示したLegacyのlazy import境界のいずれかへ解決されなければなりません。新しいimportがこの境界外へ増えた場合は、未追跡dependencyとして黙って受け入れずfail closedで拒否します。
...
Legacy adapterはSelf-host製品identityへ含めず、明示的にLegacy pathを選んだ場合だけ使うexactなdynamic import境界からloadします。そのためSelf-host MVPのload pathがLegacy実装を隠れたruntime dependencyとして持つことはありません。無関係なLegacy、documentation、governanceだけの変更ではSelf-host製品が変わっていない限り観測期間をresetしません。一方、固定Host境界またはbind済みproject-build runtime closureが変われば製品identityも変わります。
