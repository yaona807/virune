# Viruneドキュメント

[English](README.md) | [日本語](README_ja.md)

## Viruneを利用する

- [クローン後の導入手順](getting-started-from-clone_ja.md)
- [言語ガイド](language-guide_ja.md)
- [CLIリファレンス](cli-reference_ja.md)
- [標準ライブラリリファレンス](standard-library_ja.md)
- [VS Code対応](vscode_ja.md)

## 厳密なRuntime contract

- [Runtime ABI v2](runtime-abi_ja.md)

Virune 1.0の規範的な言語仕様は[`../spec/`](../spec/)にあります。

## 安定性と相互運用

- [Compiler APIの安定性](compiler-api_ja.md)
- [Compiler architecture](compiler-architecture_ja.md)
- [Self-hosting architecture](self-hosting-architecture_ja.md)
- [Stage 0セルフホスティングSeed](self-hosting-seed_ja.md)
- [セルフホスティング差分検証harness](self-hosting-differential_ja.md)
- [セルフホスティングKernel data model](self-hosting-kernel-model_ja.md)
- [セルフホスティングCompiler MVP](self-hosting-mvp_ja.md)
- [セルフホスティングFrontend](self-hosting-frontend_ja.md)
- [Incremental compilation](incremental-compilation_ja.md)
- [Fuzzing](fuzzing_ja.md)
- [TypeScript binding対応範囲](ffi-coverage_ja.md)
- [Release channel](release-channels_ja.md)
- [Stable release gate](stable-release-gate_ja.md)
