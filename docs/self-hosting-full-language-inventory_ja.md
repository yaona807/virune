...
CI logには`SELFHOST_FULL_LANGUAGE_INVENTORY`で始まるmachine-readable JSONを出力する。このJSONを、canonical project metadata、declaration／type、expression／control-flow、effect／async、runtime／deriveの独立レーンへ残作業を分割する入力として使う。各blockerの解消とfull source setの受理に応じて、このtestは削除または成功条件へ反転する。
