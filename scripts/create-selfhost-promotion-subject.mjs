...
export async function createRequiredSelfhostPromotionSubject({
	...
	const releaseCoreBytes = await readFile(resolveInside(root, releaseCorePath, 'releaseCorePath'));
	const bindings = validateReleaseCore(parseJsonObject(releaseCoreBytes, 'release-core'));
	const seedManifestFile = resolve(root, '.github/self-hosting/stage0-seed.json');
	...
	const compilerDist = resolve(root, 'packages/compiler/dist/src');
	const components = [
		...
		{ id: 'runtime-artifact', sha256: (await hashPackageProductSurface({ packageRoot: resolve(root, 'packages/runtime'), claim: 'runtime-product-v1' })).sha256 },
		{ id: 'selfhost-host-contract', sha256: (await hashFixedModuleSet({ baseDirectory: compilerDist, files: REQUIRED_SELFHOST_HOST_FILES, claim: 'required-selfhost-host-execution-contract-v1' })).sha256 },
		{ id: 'selfhost-stage3', sha256: bindings.stage3Sha256 },
		{ id: 'stdlib-artifact', sha256: (await hashPackageProductSurface({ packageRoot: resolve(root, 'packages/stdlib'), claim: 'stdlib-product-v1' })).sha256 },
	];
...
export async function hashPackageProductSurface({ packageRoot, claim }) {
	...
	const packageBytes = await readFile(resolveInside(root, 'package.json', 'package.json'));
	...
	const artifact = await hashArtifactTree(resolveInside(root, 'dist/src', 'dist/src'), `${claim}-dist-src`);
	const manifest = {
		version: 1,
...
export async function hashFixedModuleSet({ baseDirectory, files, claim }) {
	...
	for (const relativePath of canonicalFiles) {
		const absolutePath = resolveInside(base, relativePath, `module ${relativePath}`);
		const stats = await lstat(absolutePath);
		if (!stats.isFile() || stats.isSymbolicLink()) throw new Error(`fixed Host contract entry must be a regular non-symlink file: ${relativePath}`);
...
export async function hashRelativeModuleClosure({ baseDirectory, roots, claim }) {
	...
	while (queue.length > 0) {
		...
		const absolutePath = resolveInside(base, relativePath, `module ${relativePath}`);
		const stats = await lstat(absolutePath);
		if (!stats.isFile() || stats.isSymbolicLink()) throw new Error(`module closure entry must be a regular non-symlink file: ${relativePath}`);
		const bytes = await readFile(absolutePath);
		...
		if (relativePath.endsWith('.js') || relativePath.endsWith('.mjs')) {
			for (const specifier of relativeModuleSpecifiers(bytes.toString('utf8'), relativePath)) {
...
export function relativeModuleSpecifiers(source, fileName = 'module.js') {
	const parsed = ts.createSourceFile(fileName, source, ts.ScriptTarget.Latest, true, ts.ScriptKind.JS);
	if (parsed.parseDiagnostics.length > 0) throw new Error(`compiled Host module ${fileName} is not valid JavaScript: ${parsed.parseDiagnostics[0].messageText}`);
	const result = new Set();
	const add = node => {
		if (node !== undefined && ts.isStringLiteralLike(node) && node.text.startsWith('.')) result.add(node.text);
	};
	const visit = node => {
...
async function resolveRelativeModuleSpecifier(base, importer, specifier) {
	...
	for (const candidate of candidates) {
		try {
			...
			return portableRelative(base, candidate);
