...
test('exact candidate rejects out-of-allowlist, development, credential-like, nested dependency, and raw-source entries', () => {
	for (const [entryPath, expected] of [
		...
		['package/dist/helper.test.cjs.map', /test artifact is forbidden/u],
		['package/dist/schema.spec.json', /test artifact is forbidden/u],
		['package/dist/control\nname.js', /control characters are forbidden in package paths/u],
...
test('exact candidate requires exports and bin targets to exist in the audited tarball', () => {
	...
	assert.throws(
		() => verifyFixture([{ registryName: '@virune/cli', releaseAsset: 'virune-npm-1.1.0.tgz', bytes: missingBin }]),
