...
function validateConfig(name, document, errors) {
	...
	for (const [index, link] of document.contact_links.entries()) {
		...
		if (typeof link.url === 'string' && !/^https:\/\//u.test(link.url)) {
...
function validateIssueForm(name, document, errors) {
	...
	const ids = new Set();
	for (const [index, entry] of document.body.entries()) {
		validateBodyEntry(name, entry, index, ids, errors);
	}
...
export function parseYamlSubset(source) {
	...
	const parser = new StrictYamlSubsetParser(lines);
...
function parseScalar(text, lineNumber) {
	if (text.startsWith('&') || text.startsWith('*') || text.startsWith('!') || text === '---' || text === '...') {
		throw yamlError(lineNumber, 'anchors, aliases, tags, and document markers are not supported');
	}
...
if (isMain) {
	...
	console.log('Verified GitHub Issue Form/config YAML subset, schema, and contact-only privacy invariants.');
}
