...
export async function runSelfhostPromotionPerformance({
	...
	for (const fixtureId of fixtureIds) {
		...
		records.push({ fixtureId, implementations, ratios, majorRegression: ratios.coldBuild > PROMOTION_PERFORMANCE_BUDGET.majorFixtureLatencyRatio || ratios.editedRebuild > PROMOTION_PERFORMANCE_BUDGET.majorFixtureLatencyRatio });
	}
	...
	const passed = aggregateRatios.coldBuild <= PROMOTION_PERFORMANCE_BUDGET.coldBuildRatio
		&& aggregateRatios.editedRebuild <= PROMOTION_PERFORMANCE_BUDGET.editedRebuildRatio
		&& aggregateRatios.peakRss <= PROMOTION_PERFORMANCE_BUDGET.peakRssRatio
		&& aggregateRatios.artifactSize <= PROMOTION_PERFORMANCE_BUDGET.artifactSizeRatio
...
export function evaluatePromotionPerformanceSamples(fixtures) {
	...
	const records = fixtures.map(item => {
		...
		return { fixtureId: item.fixtureId, implementations: { legacy, selfhost }, ratios, majorRegression: ratios.coldBuild > PROMOTION_PERFORMANCE_BUDGET.majorFixtureLatencyRatio || ratios.editedRebuild > PROMOTION_PERFORMANCE_BUDGET.majorFixtureLatencyRatio };
	});
	const legacyAggregate = summarizeFixtureMedians(records, 'legacy');
	const selfhostAggregate = summarizeFixtureMedians(records, 'selfhost');
	...
	return {
		...
		selfhostAggregate,
		ratios,
		passed: ratios.coldBuild <= PROMOTION_PERFORMANCE_BUDGET.coldBuildRatio
			&& ratios.editedRebuild <= PROMOTION_PERFORMANCE_BUDGET.editedRebuildRatio
			&& ratios.peakRss <= PROMOTION_PERFORMANCE_BUDGET.peakRssRatio
			&& ratios.artifactSize <= PROMOTION_PERFORMANCE_BUDGET.artifactSizeRatio
			&& records.every(record => !record.majorRegression),
	};
}
