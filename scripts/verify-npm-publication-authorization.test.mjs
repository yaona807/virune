...
function publicationManifest(candidateVersion = version) {
	return {
		schemaVersion: 1,
		version: candidateVersion,
		githubReleaseTag: `v${candidateVersion}`,
		publishSource: 'reviewed-release-registry-candidate-tarball',
		bundledCliReleaseAsset: `virune-${candidateVersion}.tgz`,
		publicationReady: true,
		registryVersionEligible: true,
		distTag: 'next',
		packages: [
			{ registryName: 'virune', releaseAsset: `virune-npm-${candidateVersion}.tgz`, sha256: '1'.repeat(64), bytes: 101 },
			{ registryName: '@virune/runtime', releaseAsset: `virune-runtime-${candidateVersion}.tgz`, sha256: '2'.repeat(64), bytes: 202 },
		],
	};
}
...
test('post-write requirements cannot be moved into the pre-write authorization contract', () => {
	for (const mutate of [
		...
		contract => { contract.preWriteRequirements.push(contract.preWriteRequirements[0]); },
	]) {
...
test('reviewed publication manifest identity must agree with the source declaration', () => {
	for (const mutate of [
		...
		manifest => { manifest.packages[0].releaseAsset = `virune-runtime-${version}.tgz`; },
...
test('release version is explicit and must agree with manifest and evidence', () => {
	assert.throws(() => evaluateNpmPublicationAuthorization(input({ releaseVersion: undefined })), /releaseVersion/u);
	assert.throws(() => evaluateNpmPublicationAuthorization(input({ releaseVersion: '1.1.0-rc.2' })), /publicationManifest\.version/u);
});
...
test('Registry-ineligible and nightly release versions cannot authorize publication', () => {
	for (const candidateVersion of ['1.0.0', '1.1.0-nightly.20260822.1']) {
		assert.throws(() => evaluateNpmPublicationAuthorization(input({ releaseVersion: candidateVersion })), /Registry-eligible/u);
	}
});

test('malformed commit and execution identities fail before authorization', () => {
	assert.throws(() => evaluateNpmPublicationAuthorization(input({ reviewedCommit: 'ABC' })), /full lowercase commit SHA/u);
	for (const bad of ['', ' has-space', 'bad id', '!bad']) {
		assert.throws(() => evaluateNpmPublicationAuthorization(input({ evidenceSetId: bad })), /evidenceSetId|evidence set identity/u);
	}
});
