...
try {
	...
	execNpmSync(['pack', '--ignore-scripts', stagingPackage, '--pack-destination', stagingRoot], { stdio: 'inherit' });
	const npmPackedCli = registryReleaseAssetNameForPackage('@virune/cli', version);
	const packedCliPath = resolve(stagingRoot, npmPackedCli);
	if (!statSync(packedCliPath).isFile()) throw new Error(`npm pack did not create ${npmPackedCli}`);
	copyFileSync(packedCliPath, resolve(out, cliPackage.file));
} finally {
	rmSync(stagingRoot, { recursive: true, force: true });
}
