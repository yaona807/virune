...
JavaScript相互運用Providerは、具体的な利用方法を解決するために必要なTypeScript宣言ファイル、プラットフォーム宣言、パッケージ情報、宣言間の依存関係を読み取ってよいものとします。これらは解析の入力であり、第三者の資料のままです。
...
将来、`.d.ts`からViruneのバインディングソースを生成するなど、変換した宣言内容をファイルとして残したり再配布したりする機能は、Direct External Semanticsとは別の配布機能です。導入する場合は、元ソースの出所、コピーするコメントやドキュメント、生成ファイルの帰属表示、再配布条件を含む明示的な設計・ライセンスレビューが必要です。Direct External Semanticsの実装都合として暗黙に追加してはいけません。
