...
- Nativeの`List`、`Map`、`Set`はViruneコードから変更できません。

## 構造的な等価性とハッシュ

Runtime ABI v2にはプロトコルレジストリがありません。等価性とハッシュは、対応している不変値に対する固定の構造演算です。比較には名前的な集約IDも含まれるため、形が同じでも別の宣言から作られた値は同じものとして扱えません。関数、リソース、Foreignハンドル、対応していない可変値は、構造比較やハッシュの対象外です。
...
すべてのタスクはスコープに属します。`parallel`と`parallel try`は現在のスコープで子タスクを開始し、必要な場合は兄弟タスクへキャンセルを通知します。すべての子タスクの状態が確定するまで待ち、報告する失敗はソースコード上の順序に基づいて決定的に選択します。Runtimeは通常のVirune APIを通じて、切り離されたタスク（detached task）を公開しません。
...
`@jsExport`のラッパーは入力値を検証し、出力値を変換します。必要な場合は末尾の省略可能な引数を渡さず、JavaScriptへ公開するNative集約値は防御的にコピーします。Foreignハンドルを検証済みのNative値として扱うことはありません。

## 公開ABIスナップショット

`packages/public-abi.snapshot.json`は、Runtime v2、Interop v2、標準ライブラリのエントリーポイントについて、パッケージのexport mapと公開宣言の範囲をレビューできる形で記録します。また、生成したJavaScriptがインポートするRuntime v2の全シンボルも記録します。
