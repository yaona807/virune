...
`[entry.parameters]` `main`は引数を取らないか、型が正確に`List<String>`である引数を1つだけ取ります。引数がある場合、そのリストには`virune run`に渡したプロジェクトパスの後に指定したプログラム引数が入ります。
...
上記はシグネチャの例であり、各宣言には有効なViruneの関数本体が必要です。
...
`[entry.diagnostic]` `main`の欠落や不正は、コンパイラ内部のエラーではなく利用者のプログラム側のエラーです。安定した診断`L5010`から`L5016`を生成し、終了コード1で終了します。内部のJavaScriptスタックトレースは表示しません。
