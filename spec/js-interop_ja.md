...
CommonJSとして実行されるモジュールからの名前付きインポートは拒否します。デフォルトインポート、名前空間インポート、またはアダプターを使用します。ブラウザやバンドラーで実際に使う実行時モジュールの解決は、バンドラーの責任です。
...
複雑なTypeScript APIは`*.interop.ts`へ分離し、固定されたTypeScript Providerで型検査してから、Viruneを実行する前にESMとして出力できます。
