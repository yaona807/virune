...
## `[interop.callable-boundaries]` 生成コールバック境界
...
生成したJavaScript関数の同一性は、Virune関数自体の同一性と正規化済み境界descriptorの組で決まります。同じ関数を同じdescriptorで繰り返し変換した場合は同じJavaScript関数オブジェクトを返し、異なるdescriptorを同じ関数へ対応付けてはいけません。バージョン付きcacheはVirune関数上の列挙されないコンパイラー内部プロパティとして保持します。この仕組みのために公開project helper、origin export、Runtime ABI entry point、FFI ABI entry pointを追加してはいけません。
