...
生成コールバック境界を使えるのは、固定したProviderがJavaScript呼び出し全体を1つに確定し、その実引数に対応するコールバック型を安全に扱えると証明できた場合だけです。証拠が欠落、古い、不正、曖昧、未解決、`unknown`、コンストラクター専用、未解決ジェネリック、任意引数・可変長引数付き、または必須プロパティを持つcallable objectである場合は安全側に失敗し、アダプターを要求しなければなりません。TypeScriptの`any`は、後述する方向限定のcallback戻り値dischargeを除いて対応しません。ViruneコンパイラーでTypeScript一般の代入互換性を再実装してはいけません。
