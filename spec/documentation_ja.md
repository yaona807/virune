# ドキュメントコメント

[英語版](documentation.md)

## `[documentation.kinds]` コメントの種類

Viruneでは、3種類の行コメントを区別します。

```virune
// 通常コメント
/// 直後の宣言を説明するドキュメントコメント
//! 現在のソースモジュールを説明するドキュメントコメント
```

`///`は、スラッシュが正確に3個並び、その直後が`/`以外の場合にだけドキュメントコメントとして扱います。`////`は通常の行コメントです。Virune 1.0にはブロックコメントやブロック形式のドキュメントコメントはありません。

## `[documentation.module]` モジュールドキュメント

連続した`//!`行のグループは、現在のソースモジュールを説明します。ファイルの先頭に置き、`import`、属性、宣言、ほかのコメントより前に記述しなければなりません。先頭の空白と空行は許可します。1つのソースモジュールに記述できるモジュールドキュメントのグループは1つだけです。

## `[documentation.declaration]` 宣言ドキュメント

連続した`///`行のグループは、直後にある、ドキュメントコメントを付けられる宣言を説明します。`///`は各行で最初の空白以外の内容でなければなりません。コメントグループと宣言の間には、空行や宣言の属性を記述できます。通常コメントまたはほかのトークンがあると、宣言との関連付けは終了します。

ドキュメントコメントを付けられる対象は以下です。

- 関数
- `record`とそのフィールド
- `enum`とそのバリアント
- `newtype`と型エイリアス
- トップレベルの`let`と`const`宣言
- `extern`ブロックと`extern`関数

`import`、テスト、パラメーター、ローカル変数、文、式には付けられません。宣言へ関連付けられていないドキュメントコメントや、対応していない対象に付けられたドキュメントコメントはコンパイルエラーです。

## `[documentation.normalization]` 本文の正規化

各行からマーカーを取り除き、その直後にASCIIスペースがある場合は最大1文字だけ取り除きます。残りの行はLFで連結し、先頭と末尾にある空のドキュメント行を取り除きます。コメントグループ全体のソース範囲は保持します。

フォーマッターは、本文が空でない`///`または`//!`の直後にASCIIスペースを1文字入れます。Markdown本文の自動折り返しは行いません。

## `[documentation.markdown]` Markdown

本文はCommonMark 0.31.2互換のMarkdownです。公式ツールは生のHTMLを描画しません。最初の段落を概要として、補完やシンボル向けUIで使用します。Hoverや生成ドキュメントでは全文を表示できます。

`Parameters`、`Returns`、`Errors`、`Panics`、`Safety`、`Examples`などの見出しは慣例であり、言語構文ではありません。Virune 1.0では、`@param`、`@return`、XMLタグ、見出し名に特別な意味を与えません。

## `[documentation.semantics]` コンパイル時の意味

ドキュメントは正規化済みのテキストとしてASTに保持します。名前解決、型検査、JavaScript出力、Runtime ABI、Stable APIの互換性スナップショットには影響しません。

公式のエディタツールは、Hover、補完、Signature Help、スニペット、ドキュメント生成のCode Actionを通じてドキュメントを提供します。

## `[documentation.diagnostics]` 診断

| コード | 条件 |
|---|---|
| `L0010` | `///`グループが対応する宣言へ関連付けられていない。 |
| `L0011` | `//!`グループがファイル先頭にない。 |
| `L0012` | `///`グループの対象が対応していない構文である。 |
| `L0013` | モジュールまたは宣言に複数のドキュメントグループがある。 |
