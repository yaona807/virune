...
連続した`///`行のグループは、直後にある対応可能な宣言を説明します。`///`は、各行で最初の空白以外の内容でなければなりません。コメントグループと宣言の間には、空行や宣言の属性を記述できます。通常コメントまたはほかのトークンがあると、宣言との関連付けは終了します。
